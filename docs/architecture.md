...
- 评估信号质量
- 评估挂单距离
- 评估不同延迟场景（本地 ~300ms vs Tokyo VPS ~2-3ms）对成交率的影响
- 在模拟撮合中反映真实延迟惩罚
- 验证多级建仓模式在低延迟下的效果
...
| 供应商 | AWS Lightsail (已验证) / Vultr / Linode |
| 配置 | 1-2 vCPU、2-4 GB RAM |
| 月成本 | ~10-30 USD |
| 目标延迟 | WebSocket 单程 < 10ms |
| 实测延迟 (2026-03-30) | Binance 2.18ms (P50: 1.41ms, P99: 5.38ms) / Bybit 3.28ms |
...
| 环节 | 本地网络（参考） | 东京 VPS（实测，2026-03-30） |
|------|-------------------|-------------------|
| 交易所事件 → 收到推送 | ~300ms | 2.18ms (Binance) / 3.28ms (Bybit) |
| 信号计算 + 风控检查 | ~10-50ms | ~10-50ms |
| 下单请求 → 到达交易所 | ~300ms | 2.18ms (Binance) / 3.28ms (Bybit) |
| **总延迟** | **~650-700ms** | **~14-103ms** |

### 10.3 低延迟解锁的能力

东京 VPS 部署（实测总延迟 ~14-103ms）相比本地部署（总延迟 ~700ms），解锁以下关键能力：

**1. 主动响应式下单**

- 本地 300ms 下：必须提前挂单等待强平砸下来，位置靠猜
- Tokyo VPS ~2-3ms 下：检测到强平信号后立即下单，反应时间缩短 100-150 倍，成交率大幅提高

**2. 精确止损**

- 本地 300ms 下：止损滑点预期 ~0.3%，每笔滑点损失 ~9 USD（3x 杠杆, 1000 USD 账户）
- Tokyo VPS 2-3ms 下：止损滑点预期 ~0.01%，每笔滑点损失 < 0.3 USD
- 盈亏平衡胜率从 ~51.6% 大幅降至 ~47%

**3. 多级建仓**

- 本地 300ms 下：三单总延迟 900ms，市场早已回弹或继续崩，不可行
- Tokyo VPS 2-3ms 下：三单总延迟 ~30-50ms，可在同一事件窗口内分批建仓，降低平均入场成本，实现多级追单

**4. 多交易对监控**

- 本地 300ms 下：WebSocket 延迟堆叠，监控 3-5 个交易对是极限
- Tokyo VPS 2-3ms 下：可同时监控 30+ 个交易对，捕获面扩大 6-10 倍
- 交易频率从每天 0-2 次可能提升到 5-20 次

**5. Orderbook 实时深度分析**

- Tokyo VPS 2-3ms 下可实时处理 Orderbook 增量更新
- 检测"买单墙被吃掉" → 判断强平规模 → 精确估计冲击深度
- 检测"卖单突然增厚" → 判断是否有更大抛压即将到来 → 准确预判反向动作
- 信号质量从"有/无强平"升级到"强平多大、还会持续多久"，精度提升 10 倍

**6. 竞争排位**

- 同类"清算猎手"中，延迟决定排队顺序
- 2-3ms vs 300ms 的差距可能决定是否能抢到最有利的成交价格
- 限价单在 Orderbook 中的排位可能更靠前，成交概率显著提升
...
在 VPS 上启动任何模式之前，必须完成以下确认（Tokyo VPS 已验证完成，2026-03-30）：

- ✅ WebSocket 连接延迟实测 < 10ms (Binance: 2.18ms avg / Bybit: 3.28ms avg, P99 < 7.31ms)
- ✅ 系统时钟与 NTP 同步，偏差 < 1ms (实际偏差 < 1ms)
- ✅ SQLite 数据库文件的磁盘 I/O 不构成瓶颈 (integrity_check 全部通过)
- ✅ 进程管理工具（`systemd`）已配置自动重启，成功启动 Observe Mode
